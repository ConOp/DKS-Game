...
public class ChestRoom : Basic_Room
{

    public ChestRoom(List<GameObject> tiles, string type, int tiles_x, int tiles_z) : base(tiles, type, 3, 3)
    {
    ...
    public override void CreateRoom(List<GameObject> tiles)
    {
        ...
        for (int i = 0; i < Tiles_number_x; i++)
        {
            for (int j = 0; j < Tiles_number_z; j++)
            {
                ...
                {
                    newtile = new Tile("Portal", tiles.Where(obj => obj.name == "Chest").First(), new Vector3(xpos, ypos, zpos));
                }
                else if (i == 0 && j == 0)
                {
